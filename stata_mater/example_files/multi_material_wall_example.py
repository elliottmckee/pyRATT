...
if __name__ == "__main__":
    ...
    # Define Walls
    AeroSurf_ALU = WallStack(   materials = "ALU6061", 
                                thicknesses = 0.01, 
                                node_counts = 14)
    
    AeroSurf_MIXED = WallStack( materials = ["ALU6061", "CARBONFIBER"], 
                                thicknesses = [0.005, 0.005], 
                                node_counts = [8, 9])
    ...
    MySimulation_ALU = Thermal_Sim_1D(AeroSurf_ALU, Flight, AirModel(),
                                x_location = 0.1, 
    ...
    MySimulation_MIXED = Thermal_Sim_1D(AeroSurf_MIXED, Flight, AirModel(),
                                x_location = 0.1, 
                                deflection_angle_deg = 7.0, 
                                t_step = 0.0020,
                                t_end = 10.0,
    ...
    # Run Simulations
    # --------------
    MySimulation_ALU.run()
    MySimulation_MIXED.run()
    ...
    plt.plot(MySimulation_ALU.y_coords, MySimulation_ALU.wall_temps[:,idx],          label = "Aluminum Wall",  color='blue') 
    plt.plot(MySimulation_MIXED.y_coords, MySimulation_MIXED.wall_temps[:,idx],      label = "Mixed Material Wall",  color='red')

    plt.legend()
    plt.xlabel("Through-Wall Coordinate (m)")
    plt.ylabel("Temeperature, K")
    plt.title(f"Through-Wall Temperature Distribution Sampled at t={MySimulation_ALU.t_vec[idx]}")
